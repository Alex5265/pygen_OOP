...
class PermaDict:
    def __init__(self, data=()):
        self.dictus = dict(data)

    def __len__(self):
        return len(self.dictus)

    def __getitem__(self, item):
        return self.dictus.get(item)

    def __setitem__(self, key, value):
        if key in self.dictus:
            raise KeyError('Изменение значения по ключу невозможно')
        else:
            self.dictus.setdefault(key, value)

    def __delitem__(self, key):
        self.dictus.pop(key, None)

    def __iter__(self):
        return iter(self.dictus.keys())

    def keys(self):
        return self.dictus.keys()

    def values(self):
        return self.dictus.values()

    def items(self):
        return self.dictus.items()
